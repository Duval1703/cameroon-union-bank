...
const quickActions = [
  { icon: 'shield-checkmark-outline', label: 'Verify\nReceipt',   color: '#059669', bg: '#ECFDF5', screen: 'Verify' },
  { icon: 'cash-outline',             label: 'Add\nSale',         color: '#D97706', bg: '#FFFBEB', screen: 'Records' },
  { icon: 'trending-down-outline',    label: 'Add\nExpense',      color: '#DC2626', bg: '#FEF2F2', screen: 'Records' },
  { icon: 'bar-chart-outline',        label: 'Analytics',         color: '#7C3AED', bg: '#F5F3FF', screen: 'Insights' },
];
...
export const HomeScreen = () => {
  ...
  const predictionTip = predictions?.recommendations?.[0] || 'Add today\'s sales, expenses, and stock records to unlock sharper AI insights.';
  ...
  const handleCollect = async (provider: MoneyProvider) => {
    if (!user?.phone) {
      Alert.alert('Phone number needed', 'Please log in again or complete your merchant profile before syncing Mobile Money data.');
      return;
    }
  ...
  const handleGenerateScore = async () => {
    ...
    if (!token) {
      Alert.alert('Login required', 'Please log in before generating your Trust Score.');
      return;
    }
    ...
    if (!response.success) {
      Alert.alert('Trust Score Failed', response.error || 'Could not generate your Trust Score.');
      return;
    }
    ...
    if (trustResponse.success) setTrustData(trustResponse.data || null);
    Alert.alert('Trust Score Updated', 'Your Mobile Money transactions have been scored successfully.');
  };
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: vs(110) }}>
            ...
            >
              ...
              <View style={styles.salesMetaRow}>
                <View style={styles.growthBadge}>
                  ...
                  <Text style={styles.growthText}>{formatFcfa(recordsSummary?.sales_week)} weekly sales</Text>
                </View>
                <Text style={styles.profitLine}>Profit: <Text style={styles.profitAmt}>{formatFcfa(recordsSummary?.profit_today)}</Text></Text>
              </View>
              <View style={styles.salesDivider} />
              <View style={styles.salesStatsRow}>
                {[
                  { label: 'Sales', value: String(recordsSummary?.sales_count_today || 0) },
                  { label: 'Expenses', value: String(recordsSummary?.expenses_count_today || 0) },
                  { label: 'Stock', value: String(recordsSummary?.stock_count_today || 0) },
                ].map((s, i) => (
        ...
        <Animated.View style={[styles.trustRow, { opacity: fadeAnim }]}>
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <LinearGradient colors={['#0D4A35', '#1B5E4B']} style={styles.trustGradient}>
              ...
              <View style={styles.trustBadge}>
                <Ionicons name="star" size={rs(10)} color="#F5A623" />
          ...
          <View style={styles.trustStats}>
            {[
              { label: 'Receipts', value: String(receiptHistory.length), icon: 'receipt-outline', color: '#059669' },
              { label: 'Records today', value: String(recordsToday), icon: 'trending-up-outline', color: '#2563EB' },
              { label: 'Alerts', value: String(suspiciousReceipts), icon: 'warning-outline', color: '#D97706' },
            ].map((s, i) => (
          ...
          <View style={styles.syncCard}>
            <View style={styles.syncHeader}>
              <View style={styles.syncIconLarge}>
                <Ionicons name="wallet-outline" size={rs(22)} color="#065F46" />
              </View>
              <View style={{ flex: 1 }}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Ionicons name="analytics-outline" size={rs(18)} color="#065F46" />
              <Text style={styles.scoreBtnText}>{scoring ? 'Generating Trust Score...' : 'Generate Trust Score'}</Text>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Ionicons name="list-outline" size={rs(18)} color="#0D4A35" />
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.88}
        >
          <LinearGradient colors={['#0D4A35', '#1B5E4B', '#2D7A62']} style={styles.insightBanner} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
            <View style={styles.insightOrb} />
            <View style={styles.insightLeft}>
              <View style={styles.insightChip}>
                <Ionicons name="sparkles" size={rs(10)} color="#F5A623" />
                <Text style={styles.insightChipText}>{predictions ? `${Math.round(predictions.confidence || 0)}% CONFIDENCE` : 'AI INSIGHT'}</Text>
              </View>
              <Text style={styles.insightTitle}>
                {predictions ? `${formatFcfa(predictions.predicted_profit_next_week)} FCFA projected profit` : 'Keep building your record history'}
              </Text>
          ...
          <View style={styles.activityCard}>
            {recentRecords.length === 0 ? (
              <View style={styles.emptyActivity}>
                ...
                <Text style={styles.emptyActivityText}>Add sales, expenses, stock, or receipts to build your dashboard.</Text>
              </View>
      ...
      <TouchableOpacity style={styles.fab} onPress={() => navigateToTab('Records')} activeOpacity={0.85}>
        <LinearGradient colors={['#F5A623', '#D97706']} style={styles.fabGrad}>
...
const styles = StyleSheet.create({
  ...
  notifDot: { position: 'absolute', top: 0, right: 0, width: rs(8), height: rs(8), borderRadius: rs(4), backgroundColor: '#F87171', borderWidth: 1, borderColor: '#0D4A35' },
  ...
  trustCard: { borderRadius: Radius.xl, overflow: 'hidden', shadowColor: '#0D4A35', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 8 },
  ...
  trustBadgeText: { fontSize: FontSize.xs, color: '#F5A623', fontWeight: FontWeight.bold, letterSpacing: 0.5 },
  ...
  seeAll: { fontSize: FontSize.base, color: '#1B5E4B', fontWeight: FontWeight.semibold },
  ...
  syncIconLarge: { width: rs(44), height: rs(44), borderRadius: rs(14), backgroundColor: '#ECFDF5', alignItems: 'center', justifyContent: 'center' },
  ...
  collectionValue: { fontSize: FontSize.base, fontWeight: FontWeight.extrabold, color: '#0D4A35' },
  ...
  scoreBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: rs(8), backgroundColor: '#ECFDF5', borderWidth: 1, borderColor: '#A7F3D0', paddingVertical: rs(12), borderRadius: Radius.lg },
  scoreBtnText: { fontSize: FontSize.sm, fontWeight: FontWeight.bold, color: '#065F46' },
  ...
  transactionsBtnText: { fontSize: FontSize.sm, fontWeight: FontWeight.bold, color: '#0D4A35' },

  // Insight banner
  insightWrap: { marginHorizontal: rs(16), marginTop: rs(16), borderRadius: Radius.xl, overflow: 'hidden', shadowColor: '#0D4A35', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 8 },
  ...
  insightChipText: { fontSize: FontSize.xs, color: '#F5A623', fontWeight: FontWeight.bold, letterSpacing: 0.5 },
  ...
  insightCTA: { fontSize: FontSize.sm, fontWeight: FontWeight.bold, color: '#F5A623' },
